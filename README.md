...
correct. This particular library has been tested with Arduino DUE, UNO and
MEGA2560.
...
## Limitations
• This library has been tested with ARDUINO DUE, UNO and MEGA2560 under windows environment, 
  it might not work with other board.
...
Make sure the dueHX711/due_HX711 library is available with listArduinoLibraries command.
```c++  
...
```c++  
3 a= arduino (’com6’,’Due’,’libraries’,’dueHX711/due_HX711’);
```  
...
```c++  
4 a= arduino (’com6’,’Due’) // If you ’ve used Hardware Setup ;
5 LoadCell = addon (a,’dueHX711/due_HX711’,’Pins’,{’D2’,’D3’})
``` 
The default gain is 128, if you wish you can use 64 for channel A and 32 for channel B with an additional parameter:
```c++  
5 LoadCell = addon (a,’dueHX711/due_HX711’,’Pins’,{’D2’,’D3’},’Gain’,64)
``` 
The serial comunication protocol is time sensitive so if you need to use Interrupt set "true" 
with an additional parameter: 
```c++  
5 LoadCell = addon (a,’dueHX711/due_HX711’,’Pins’,{’D2’,’D3’},’Interrupt’,true)
``` 
